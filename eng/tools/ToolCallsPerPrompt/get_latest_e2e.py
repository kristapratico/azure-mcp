...
def load_test_settings_from_file(test_settings_path=".testsettings.json", service_name=None):
    ...
    try:
        ...
        if service_name:
            ...
            if azure_mcp_root:
                service_test_settings = os.path.join(
                    azure_mcp_root, "areas", service_name, "tests", ".testsettings.json"
                )
...
def main():
    ...
    parser.add_argument(
        "--service",
        type=str,
        help=f'Filter results to only include entries for the specified service area(s). Use comma-separated values for multiple services. Available services: {", ".join(sorted(set(service_header_mapping.values())))}',
    )
    parser.add_argument(
        "--output", "-o", type=str, default="data.jsonl", help="Output JSONL file name (default: data.jsonl)"
    )
    ...
    source_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "e2eTests", "e2eTestPrompts.md"
    )
    ...
    if service_names and not jsonl_data:
        print(
            f"⚠️  WARNING: No entries found for services '{', '.join(service_names)}'. Please check the service name spelling."
        )
    ...
    processed_data, unmapped_placeholders, skipped_count = replace_placeholders_and_track_unmapped(
        jsonl_data, current_variable_mappings, service_names
    )
