...
options = {
    ...
    "storage account": "aifoundrystorageshared",
    "cosmos resource": "archagent-cosmos",
    "database": "guidelines",
    "appconfig": "appconfig",
    "search resource": "mysearch",
    "index": "mysvc",
    "kusto cluster": "mycluster",
    "kusto db name": "db1",
    "storage container": "bloby",
    "cluster uri": "https://mycluster.kusto.windows.net",
    "account-name": "my-cosmos-account",
    "sub id": "faa080af-c1d8-40ad-9cce-e1a450ca5b57",
}
...
def reshape_tools(tools) -> list[dict]:
    return [{
        "type": "tool_call",
...
def reshape_tool_definitions(available_tools) -> list[dict]:
...
async def call_mcp_tool(tool_call, function_args):
...
def make_request(response, messages, tools, tool_calls_made):
    answer = None
...
def evaluate_azure_mcp(query: str, expected_tool_calls: dict):
    ...
    while True:
        ...
        response = client.chat.completions.create(
            model=MODEL,
            messages=[{"role": "system", "content": "You are a helpful user."},
    ...
    if response.tool_calls is not None:
        ...
        messages, tool_calls = make_request(response, messages, available_tools, tool_calls_made)
...
if __name__ == "__main__":
    ...
    test_file = pathlib.Path(__file__).parent / "test.jsonl"
    result = evaluate(
        data=str(test_file),
        evaluators={
